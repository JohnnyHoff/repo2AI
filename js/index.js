	...
	.addEventListener("submit", async function (e) {
		...
		try {
			...
			if (lastString) {
				...
				const matchingRef = allRefs.find((ref) =>
					lastString.startsWith(ref),
				);
				if (matchingRef) {
			...
			document.getElementById("generateTextButton").style.display =
				"flex";
			...
			fileItems.forEach(async (item) => {
				...
				try {
					...
					if (typeof GPTTokenizer_cl100k_base !== "undefined") {
						...
						const fileLi = document
							.getElementById(spanId)
							?.closest("li");
						if (fileLi) {
							fileLi.dataset.tokenState = tokenCount; // Store count on li
							const tokenSpan = fileLi.querySelector(
								':scope > span[id^="token-count-"]',
							);
							if (tokenSpan) {
								tokenSpan.textContent = ` (Tokens: ${tokenCount})`;
								tokenSpan.style.color = ""; // Reset color
							}
							// Trigger update starting from parent folder
							const parentLi =
						...
						console.warn(
							"GPT Tokenizer not loaded when trying to count tokens for",
							item.path,
						);
						const fileLi = document
							.getElementById(spanId)
							?.closest("li");
						if (fileLi) {
							fileLi.dataset.tokenState = "Error"; // Store error state
							const tokenSpan = fileLi.querySelector(
								':scope > span[id^="token-count-"]',
							);
							if (tokenSpan) {
								tokenSpan.textContent = ` (Tokenizer Error)`;
								tokenSpan.style.color = "orange";
							}
							// Trigger update starting from parent folder
							const parentLi =
								fileLi.parentElement?.closest("li");
					...
					console.error(
						`Failed to fetch/process token count for ${item.path}:`,
						error,
					);
					const fileLi = document
						.getElementById(spanId)
					...
					if (fileLi) {
						...
						if (tokenSpan) {
							tokenSpan.textContent = ` (Fetch Error)`; // Indicate error
							tokenSpan.style.color = "red";
						}
						// Trigger update starting from parent folder
						const parentLi = fileLi.parentElement?.closest("li");
						if (parentLi) {
							updateFolderTokenCounts(parentLi);
						}
					}
			...
			// Initial calculation for all top-level folders after files start loading
			document
				.querySelectorAll(
					"#directoryStructure > ul > li > input.directory-checkbox",
				)
				.forEach((cb) => {
					updateFolderTokenCounts(cb.closest("li"));
				});
		} catch (error) {
			outputText.value =
...
// Add this helper function to fetch single file content
...
document
	.getElementById("generateTextButton")
...
function updateInfoIcon(button, tokenInfo) {
	...
	if (icon) {
		icon.setAttribute(
			"data-lucide",
			tokenInfo.classList.contains("hidden") ? "info" : "x",
		);
		lucide.createIcons();
	}
}
